...
- **Schema source:** `apps/api-gateway/schema.graphql` (single source of truth, auto-generated by gateway)
...
**All frontend UI MUST follow the rules in `.claude/rules/ui-baseline.md`.** That file is the enforced source of truth for visual consistency — HeroUI v3 components, color system, spacing, typography, and patterns.

Key points:
- Use HeroUI components (`Button`, `Input`, `Card`, `Tabs`, `Chip`, `Avatar`, `Skeleton`, etc.) — never raw HTML elements with Tailwind
- Use HeroUI semantic colors (`color="primary"`, `text-default-*`) — not Tailwind color utilities (`bg-green-600`, `text-neutral-*`)
- Follow standardized spacing, typography, and loading patterns defined in `ui-baseline.md`
...
## Public Pages with Apollo Client

The `(public)` layout has no providers. Public pages that need Apollo Client (e.g., the portfolio page at `/u/[userId]`) wrap their content with `<AppProviders>` inline:

```typescript
// src/app/(public)/u/[userId]/page.tsx
'use client';
...
This creates a separate Apollo Client instance (not shared with `(app)` layout), which is acceptable since public pages have no shared cache needs with authenticated pages.

## Next.js App Router Conventions

- Route groups: `(public)` for unauthenticated pages, `(app)` for authenticated pages
- Public profile at `/u/[userId]` is in the `(public)` group — no auth required
...
- Violate the UI baseline (`.claude/rules/ui-baseline.md`) — use HeroUI components, not raw HTML + Tailwind
