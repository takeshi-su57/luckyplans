...
import { chains, protocols, type Chain, type Protocol } from './data/protocols';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        {Icon && <Icon size={28} />}
        <div>
          <span className="text-base font-bold text-[#37352f]">{protocol.name}</span>
          <span className="ml-2 text-xs font-medium text-[#a3a29e]">{protocol.ticker}</span>
        </div>
      </div>
      <p className="mt-3 text-sm leading-relaxed text-[#787774]">{protocol.description}</p>
      <div className="mt-4 flex flex-wrap gap-3">
        <StatPill label="Volume" value={protocol.stats.totalVolume} />
        <StatPill label="Leverage" value={protocol.stats.maxLeverage} />
        {protocol.stats.pairs && <StatPill label="Pairs" value={protocol.stats.pairs} />}
        {protocol.stats.users !== '—' && <StatPill label="Users" value={protocol.stats.users} />}
      </div>
...
function ChainCard({ chain }: { chain: Chain }) {
  ...
  return (
    <a
      ...
      className="group flex items-start gap-3 rounded-lg border border-[#e8e7e4] bg-white p-4 transition-colors hover:border-[#d9d8d4]"
    >
      <div
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-[#37352f]">{chain.name}</span>
          <span className="rounded bg-[#f1f1ef] px-1.5 py-0.5 font-mono text-[10px] font-bold text-[#37352f]">
            {chain.stat.value}
          </span>
        </div>
        <p className="mt-0.5 text-xs leading-relaxed text-[#787774]">{chain.description}</p>
      </div>
    </a>
  );
}
    ...
    <SectionContainer id="chains">
      <div className="flex flex-col gap-4">
        ...
        <p className="max-w-2xl text-base text-[#787774]">
          Indexing trader activity across major perpetual DEX protocols and EVM chains. Real data.
          Real volume. Real infrastructure.
        </p>
      </div>

      <div className="mt-12 space-y-4">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-[#787774]">Protocols</h3>
      ...
      <div className="mt-12 space-y-4">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-[#787774]">Chains</h3>
