...
Builds Docker images for all deployable artifacts in parallel via a matrix strategy:

| Service        | Dockerfile                    | Image                            |
| -------------- | ----------------------------- | -------------------------------- |
| landing        | `apps/landing/Dockerfile`     | `ghcr.io/<owner>/landing`        |
| docs           | `apps/docs/Dockerfile`        | `ghcr.io/<owner>/docs`           |
| web            | `apps/web/Dockerfile`         | `ghcr.io/<owner>/web`            |
| api-gateway    | `apps/api-gateway/Dockerfile` | `ghcr.io/<owner>/api-gateway`    |
| prisma-migrate | `packages/prisma/Dockerfile`  | `ghcr.io/<owner>/prisma-migrate` |
...
2. Uses `yq` to update image tags in `values.prod.yaml` for `landing`, `docs`, `web`, `api-gateway`, and `prisma-migrate`
3. Commits with `[skip ci]` to prevent re-triggering CI
4. Pushes to `main`

ArgoCD then detects the commit to `values.prod.yaml` and auto-syncs the new image tags.
...
See [ArgoCD](/architecture/argocd) for full operational documentation.
