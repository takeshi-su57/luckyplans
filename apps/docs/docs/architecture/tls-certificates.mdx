...
| Record | Name    | Value              |
| ------ | ------- | ------------------ |
| A      | `beta`  | `<your-server-ip>` |
| A      | `api`   | `<your-server-ip>` |
| A      | `admin` | `<your-server-ip>` |

Port **80** must be reachable from the internet.

`v0.api.luckyplans.xyz` is routed by ingress to the `legacy-v0-api` in-cluster proxy, which forwards to the host-level legacy API at `host.k3d.internal:9000`. Production values add a pod `hostAliases` entry for the Docker host gateway; public DNS and TLS are still required like the other hosts.
...
kubectl -n luckyplans describe certificate beta-luckyplans-tls
...
# 4. Test HTTPS
curl -v https://beta.luckyplans.xyz
...
kubectl -n luckyplans get secret beta-luckyplans-tls -o yaml > beta-luckyplans-tls-backup.yaml
