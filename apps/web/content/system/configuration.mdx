...
| `WORKER_CREDENTIAL_PEPPER` | —       | Secret pepper used by the gateway when hashing/verifying worker (edge) credential material. Generate with `openssl rand -base64 48`. Treat as high-sensitivity secret and manage via Sealed Secrets in production. |
...
> **Note:** The product web app no longer requires any auth-related environment variables. In local development Apollo Client uses a relative `/graphql` URL. In production the web image build arg points at `https://api.luckyplans.xyz/graphql`. The landing SPA has its own build-time URLs for app/docs navigation.
...
| `ingress.*`       | Traefik ingress configuration (host routing for `landing`, `app`, `api`, `admin`, `v0.api`) |
...
| `landing.*`       | Landing SPA deployment config and baked `appUrl`/`docsUrl` build args |
