...
> **Note:** The frontend no longer requires any auth-related environment variables. In local development Apollo Client uses a relative `/graphql` URL. In production the web image build arg points at `https://api.luckyplans.xyz/graphql`.
...
| `ingress.*`       | Traefik ingress configuration (host routing for `app`, `api`, `admin`, `v0.api`) |
