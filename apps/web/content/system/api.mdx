...
- **Prod URL:** `https://api.luckyplans.xyz/graphql`
...
These endpoints are served by the API Gateway for trusted edge runtime communication.

### Internal Edge Registration

- **Endpoint:** `POST /internal/edges/register`
- **Purpose:** Register an edge device and issue worker credentials tied to the device identity.

### Internal Edge Connectivity Heartbeat

- **Endpoint:** `POST /internal/edges/connectivity`
- **Purpose:** Report edge connectivity/last-seen status and receive upgrade intent metadata.
