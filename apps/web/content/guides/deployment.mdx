...
3. Register the release in LuckyPlans using the current `createEdgeRelease` mutation:
   ...
   - `checksum`: checksum for the primary registered artifact
   - `signature`: signature for that checksum
...
The current gateway mutation preserves the legacy top-level `windowsUrl`, `linuxUrl`, `checksum`, and `signature` fields while backfilling platform-specific artifact rows. Keep the manifest with the release record so operators can compare the registered metadata against the CI-produced artifact set.
