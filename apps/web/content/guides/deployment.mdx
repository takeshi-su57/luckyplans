...
- **Grafana**: `kubectl -n monitoring port-forward svc/grafana 3002:3000` → http://localhost:3002
...
```bash
# Rebuild and redeploy only the landing SPA:
...
| `WORKER_CREDENTIAL_PEPPER` | API Gateway pepper for worker credential hashing/verification |
...
Release metadata is signed before registration. CI requires these GitHub Actions secrets:

| Secret                             | Purpose                                                                 |
| ---------------------------------- | ----------------------------------------------------------------------- |
| `EDGE_RELEASE_SIGNING_PRIVATE_KEY` | Ed25519 private key in PEM format used to sign artifact SHA-256 values. |
| `EDGE_RELEASE_SIGNING_KEY_ID`      | Stable key identifier written into release metadata for auditability.   |

The API gateway verifies registered release signatures with `EDGE_RELEASE_SIGNING_PUBLIC_KEY`. Never commit either signing key to git.

### Artifact publishing flow

Use the **Edge Agent Release** GitHub Actions workflow to build release assets. Manual runs require a semver `version` and `signing_key_id`. Tag runs publish assets when the pushed tag matches `edge-agent-v*`.

The workflow produces:

- `luckyplans-edge-agent-v<version>-linux-x64-service.tar.gz`
- `luckyplans-edge-agent-v<version>-win32-x64-service.zip`
- detached `.sig` files for each artifact
- `SHA256SUMS`
- `manifest.json`

The generated manifest contains gateway-compatible artifact metadata:
...
EDGE_RELEASE_SIGNING_PRIVATE_KEY="$(cat ./edge-release-private-key.pem)" \
  pnpm --filter @luckyplans/edge-agent release:build -- \
  --version 1.2.3 \
  --base-url https://github.com/takeshi-su57/luckyplans/releases/download/edge-agent-v1.2.3 \
  --signing-key-id edge-release-2026-06
```

### Release registration flow

1. Build and publish edge artifacts with the **Edge Agent Release** workflow.
2. Open the generated `manifest.json`.
3. Register the release in LuckyPlans using the current `createEdgeRelease` mutation:
   - `version`: manifest `version`
   - `windowsUrl`: `win32` artifact `url`
   - `linuxUrl`: `linux` artifact `url`
   - `checksum`: `win32` artifact checksum for legacy top-level release compatibility
   - `signature`: signature for that legacy checksum
   - `artifacts`: manifest `artifacts` array, preserving each platform-specific `checksum`, `signature`, `signatureAlgorithm`, `signingKeyId`, and `sizeBytes`
   - `notes`: optional operator notes
4. Confirm `edgeReleases` returns artifact records for `linux/x64/service` and `win32/x64/service`.
5. Set `targetVersion` for selected workers or start an upgrade campaign.

The current gateway mutation preserves the legacy top-level `windowsUrl`, `linuxUrl`, `checksum`, and `signature` fields while accepting platform-specific artifact rows from the manifest. Edge upgrade metadata resolution uses the artifact rows, so each platform must keep the checksum and signature for the exact package bytes it installs.
...
5. Gateway upgrade campaigns advance or pause from edge heartbeat-reported terminal statuses without requiring a separate operator status mutation.
