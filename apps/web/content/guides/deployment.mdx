...
Traefik ingress
  /            -> landing:8080
  /login       -> web:3000
  /graphql     -> api-gateway:3001
  /auth/*      -> api-gateway:3001
  /uploads/*   -> api-gateway:3001
...
docker build --build-arg NEXT_PUBLIC_GRAPHQL_URL="/graphql" -t luckyplans/web:latest -f apps/web/Dockerfile .
...
```text
Push to main -> CI build and push -> update image tags -> ArgoCD sync -> smoke tests
```
...
- DNS A records for `luckyplans.xyz`, `app.luckyplans.xyz`, `api.luckyplans.xyz`, `admin.luckyplans.xyz`, and `v0.api.luckyplans.xyz`
