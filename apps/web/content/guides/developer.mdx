...
| ------------ | ------------- | ----------------------------- | --------------------------------------- |
| Landing SPA  | 3000          | http://localhost:3000         | Vite marketing site                     |
| Frontend     | 3000          | http://localhost:3000/login   | Next.js product app (same ingress host locally) |
...
4. Register a new account or log in with the test user:
   - **Email:** `testuser@luckyplans.xyz`
   - **Password:** `password`
5. After login, you should be redirected back to the app with a valid session
...
| deploy-local.sh landing     | Rebuild and redeploy the landing SPA                     |
| deploy-local.sh web         | Rebuild and redeploy the product web app                 |
