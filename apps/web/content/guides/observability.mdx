...
- Legacy API remains available at `https://v0.api.luckyplans.xyz` through the `legacy-v0-api` in-cluster proxy
...
1. `lastSeenAt` freshness for each worker (connectivity heartbeat health)
2. `version` vs `targetVersion` drift (pending upgrade intent)
3. `upgradeStatus` transitions:
   - `UPGRADE_PENDING`
   - `DOWNLOADING`
   - `VERIFYING`
   - `RESTARTING`
...
# Gateway edge lifecycle transition logs
{service_name="api-gateway"} |= "edge.upgrade.status_transition"
{service_name="api-gateway"} |= "edge.upgrade.target_assigned"

# Edge-agent lifecycle logs collected from service stdout/stderr
{service_name=~"edge-agent|luckyplans-edge-agent"} |= "edge.daemon.started"
{service_name=~"edge-agent|luckyplans-edge-agent"} |= "edge.heartbeat.failed"
{service_name=~"edge-agent|luckyplans-edge-agent"} |= "edge.upgrade.status_reported"
{service_name=~"edge-agent|luckyplans-edge-agent"} |= "edge.runtime.shutdown_requested"
...
1. **Stale `lastSeenAt`**: worker likely offline, blocked, or unable to reach gateway.
2. **`targetVersion` set but status never leaves `UPGRADE_PENDING`**: worker is likely busy (idle-only upgrade policy) or connectivity is unstable.
3. **Frequent `FAILED` upgrades**: verify release URLs, checksum/signature metadata, and edge host permissions.
4. **`ROLLED_BACK` status**: check the recovery state path, failed target marker, and service restart logs.
5. **`QUARANTINED` worker**: inspect the last three task failures. A successful task completion resets the worker failure counter; three consecutive failures quarantine the worker.

For step-by-step operator procedures, see [Edge Lifecycle Operator Runbooks](/docs/guides/developer#edge-lifecycle-operator-runbooks).
...
> **Note:** Alert _notifications_ (email, Slack, PagerDuty) are not yet configured. Alerts currently only show in the Prometheus UI.
