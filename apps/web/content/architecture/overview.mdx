...
|---------|---------|---------------|
| Keycloak | Identity Provider — user management, roles | ROPC + Admin API (gateway ↔ Keycloak server-side) |
...
- **Local dev:** `docker compose up -d` (Redis, Keycloak), then `pnpm dev`
...
- **Auth:** Keycloak (self-hosted), gateway-managed sessions with Redis store
...
- **Keycloak required** — local development needs `docker compose up -d` before `pnpm dev`
