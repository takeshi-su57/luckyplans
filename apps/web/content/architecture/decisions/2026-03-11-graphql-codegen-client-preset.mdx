---
title: Switch GraphQL Codegen to client-preset with Inline Operations
description: Replaced three separate codegen plugins and .graphql files with client-preset and inline graphql() calls.
---
...
- The `graphql()` function uses string literal type matching — re-run codegen after any operation change
...
- The generated `gql.ts` includes a string map of all operations, which is not tree-shakeable without the babel/swc plugin (acceptable at current scale).
