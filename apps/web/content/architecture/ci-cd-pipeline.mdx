...
Builds Docker images for all three deployable artifacts in parallel via a matrix strategy:
...
2. Uses `yq` to update image tags in `values.prod.yaml` for `web`, `api-gateway`, and `prisma-migrate`
