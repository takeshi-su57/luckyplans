...
Legacy service note: `v0.api.luckyplans.xyz` is routed by this Helm chart to an in-cluster `legacy-v0-api` proxy service. The proxy pod forwards traffic to the host-level legacy API at `host.k3d.internal:9000`, avoiding Traefik's default `ExternalName` service restrictions while keeping the legacy process outside Kubernetes.
