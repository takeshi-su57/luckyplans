...
`v0.api.luckyplans.xyz` is routed by ingress to the `legacy-v0-api` in-cluster proxy, which forwards to the host-level legacy API at `host.k3d.internal:9000`. It still requires DNS and TLS like the other hosts.
