...
How to maintain and evolve the AI engineering framework (`.agents/`, `AGENTS.md`, related docs).

## Framework File Hierarchy

```
AGENTS.md              ← Human-readable overview (entry point for developers)
.agents/
├── AGENTS.md                  ← AI context (auto-loaded, concise index, < 200 lines)
...
1. Update `.agents/rules/architecture.md` — the source of truth
2. Update `AGENTS.md` — the summary in Architecture Patterns section
3. Update `AGENTS.md` — the Core Engineering Principles section
4. Update `README.md` — only if it affects project structure, tech stack, or commands

### When adding a new rule

1. Create `.agents/rules/<topic>.md`
2. Add one-line reference in `AGENTS.md` under Rules section
3. Add row in `AGENTS.md` Framework Structure table

### When adding a new skill

1. Create `.agents/skills/<skill-name>/` with `SKILL.md`, `templates/`, `examples/`
2. Add row in `AGENTS.md` Framework Structure table
3. No change needed in `AGENTS.md` unless it's a frequently used skill

### When modifying shared types or message patterns

1. Update `packages/shared/src/types/index.ts`
2. Check if `.agents/rules/architecture.md` examples still match
3. Check if skill examples still match

### When authentication or session management changes

1. Update `.agents/rules/security.md` — Authentication section (source of truth for auth rules)
2. Update `.agents/rules/architecture.md` — Gateway auth section
3. Update `.agents/rules/frontend.md` — if frontend auth patterns change (middleware, hooks, Apollo config)
4. Update `AGENTS.md` — Architecture Patterns summary
...
6. Update `AGENTS.md` — Repository Layout and Architecture Patterns if new services/containers
...
2. Update `AGENTS.md` Key Commands section
...
1. Update `.agents/rules/frontend.md` — if provider order, Apollo config, or auth pattern changed
2. Verify `pnpm build` passes — Next.js prerendering is sensitive to layout/provider changes
3. Update `.agents/rules/architecture.md` — Frontend section if routing structure changed
...
- [ ] `.agents/rules/architecture.md` — if patterns changed
...
- [ ] `AGENTS.md` — Tech Stack, Repository Layout, Architecture Patterns, Key Commands, Known Gaps
...
`AGENTS.md` is loaded into every AI interaction. It must be:
...
- **No duplicated content** — if it's in a rule file, AGENTS.md links to it, doesn't repeat it
...
- Every new file must be referenced from the appropriate parent (AGENTS.md or AGENTS.md)
...
- Periodically check that AGENTS.md line count is under 200
...
- Clean up references in AGENTS.md and AGENTS.md when removing
...
- Every rule file is referenced in AGENTS.md or AGENTS.md
...
- Known Gaps in `AGENTS.md` are accurate — remove items that have been implemented
